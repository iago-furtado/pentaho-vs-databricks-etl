...
# MAGIC %md
# MAGIC # Gold to delivery artifacts
# MAGIC
# MAGIC This notebook creates a delivery-ready CSV and PDF from the Gold monthly-sales dataset.
...
delivery_start_time = perf_counter()
...
summary = gold_sales.agg(
    F.count("*").alias("delivery_row_count"),
    F.sum("transaction_count").alias("aggregated_transaction_count"),
...
story = [
    ...
    Paragraph(
        f"Scenario: {SCENARIO} | Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
        styles["Normal"],
    ),
    Spacer(1, 0.4 * cm),
...
kpi_data = [
    ["Metric", "Value"],
    ["Transactions", f"{summary['aggregated_transaction_count']:,}"],
...
story.extend([
    Paragraph("Sales by month", styles["Heading2"]),
    build_table(monthly_data, [3 * cm, 3 * cm, 9 * cm]),
    Spacer(1, 0.45 * cm),
])
...
delivery_summary = spark.createDataFrame(
    [
        (
            SCENARIO,
            GOLD_TABLE,
            int(summary["delivery_row_count"]),
            int(summary["aggregated_transaction_count"]),
            str(summary["total_sales_amount"]),
            round(delivery_elapsed_seconds, 3),
            CSV_REPORT_PATH,
            PDF_REPORT_PATH,
        )
    ],
    [
        ...
        "source_table",
        "delivery_row_count",
        "aggregated_transaction_count",
        "total_sales_amount_sum",
        "elapsed_seconds",
        "csv_report_path",
        "pdf_report_path",
    ],
